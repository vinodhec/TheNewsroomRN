...
const uploadFile = (file, fullFilePath) => {
  ...
  console.log({ config }, fullFilePath)
  const storageRef = ref(config.storage, fullFilePath);
...
const deleteFile = (fileDownloadUrl) => {
  ...
  const refToDelete = ref(config.storage, filepath)
...
const uploadSingleImage = (image) => {
  ...
  return fetch(uri).then(async (image64) => {
    ...
    return uploadFile(
      imgBlob,
      `${config.STORAGE_PATH.GROUPS}/${imageArti?.fileName}`,
    )
  })
}
