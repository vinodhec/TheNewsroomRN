...
const BreakingNews = ({ imageUrl }) => {
  ...
  return (
   breaking?.id && <View
      style={{ position: 'relative', backgroundColor: '#FAE9EA', marginBottom: -20 }}
      className="py-2">
      <ImageBackground
        style={{
          width: 51,
      ...
      <PressableOpacity onPress={async () => {
        ...
        await FirestoreService.deleteDocument(COLLECTIONS.BREAKING, breaking?.id.replaceAll('breaking/', ""));
